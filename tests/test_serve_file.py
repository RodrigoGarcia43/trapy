...
class TestServeFile(unittest.TestCase):
    ...
    def test_download_small(self):
        ...
        h1.cmdPrint(
            "{} -mserve_file --accept {} --file {} &".format(
                config.PYTHON, address, server_file
            )
        )
        # wait_for(partial(is_port_open, address, h1))
        h2.cmdPrint(
            "{} -mserve_file --dial {} --file {}".format(
                config.PYTHON, address, client_file
            )
        )
        ...
        hashes = set(file_hashes(server_file, client_file).values())
